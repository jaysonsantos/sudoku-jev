...
- On `/chess` the browser randomly assigns Jev one color and Stockfish the other. Only Jev's turn uses `/ws`. Stockfish is the lite single-thread WASM build and must not run as a native binary on the Worker.
